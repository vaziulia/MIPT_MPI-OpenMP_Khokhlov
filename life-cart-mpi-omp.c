...
int main(int argc, char **argv)
{
	...
	if (argc != 3) {
		printf("Usage: %s input file, set_num_thr.\n", argv[0]);
		return 0;
	}
	life_t l;
	life_init(argv[1], &l);
	int set_num_thr = strtol(argv[2], NULL, 10);

	omp_set_num_threads(set_num_thr);
#pragma omp parallel
{
...
*/
    ...
    if (rank == size - 1) {
        ...
        // запись в файл
        FILE *fptr;
        fptr = fopen("mpi-runtime-task7.txt", "a");
        fprintf(fptr, "%d, %d, %d, %f\n", l.size * num_threads, l.size, num_threads, dt);
        fclose(fptr);
    }
